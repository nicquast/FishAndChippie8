...
Chip8System initChip8System() {
  Chip8System system = {.memory = initMemory(),
...
bool instructionTick(Chip8System *system, DisplayHandle display_handle) {
  ...
  case 0xB:
    ...
    system->register_store.program_counter =
        nnn + system->register_store.gp_registers[0];
    break;
  case 0xC:
    ...
    break;
  case 0xD:
    // Display - DXYN - Draw an N pixel tall sprite from memory location at I
    // index register to the screen at X coord at VX and Y coord at VY, set
    // pixels by flipping them. If we flip one from on to off by doing this set
    // VF to 1.
  ...
  case 0xE:
    ...
    break;
  case 0xF:
    // Timers add to index, get key, font, binary-coded decimal conversion,
    // store and load memory
    //  Lots of stuff going on with this first nibble...
    switch (nn) {
    case 0x07:
      system->register_store.gp_registers[x] =
          system->register_store.delay_timer;
      break;
    case 0x15:
      system->register_store.delay_timer =
          system->register_store.gp_registers[x];
      break;
    case 0x18:
      system->register_store.sound_timer =
          system->register_store.gp_registers[x];
      break;
    case 0x1E:
      // Add VX to Index register
      // Implementing Amiga interpreter to retain compatibility wit Spaceflight
      // 2091!
    ...
    case 0x33:
      ...
      break;
    case 0x55:
      // Store successive registers up to x into memory starting at the index
      // register
      for (int i = 0; i <= x; i++) {
        const mem_addr_t save_address =
            system->register_store.index_register + i;
    ...
    case 0x65:
      ...
      // NOTE: Original COSMAC interpreter did increment, modern interpreters do
      // not. default behaviour should be the more modern approach
      for (int i = 0; i <= x; i++) {
        const mem_addr_t load_address =
            system->register_store.index_register + i;
